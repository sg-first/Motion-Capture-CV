...
def binary_crossentropy(args):
    y_true, y_pred,put=args
    out = K.mean(K.binary_crossentropy(y_true, y_pred), axis=-1)
    return out

def binary_crossentropy1(args):
    y_true,y_pred1,y_pred2,put=args
    x=K.mean(put[:,0,0])
    y=K.variable(1)
    out=K.switch(K.equal(x, y), K.mean(K.binary_crossentropy(y_true, y_pred2), axis=-1),
                            K.mean(K.binary_crossentropy(y_true, y_pred1), axis=-1))
    return out
...
#计算损失函数层
loss_out1 = keras.layers.Lambda(binary_crossentropy, output_shape=(1,))([input1, out_arm_L,input]) # output_shape是函数返回值的shape，第一个维度是占位符
loss_out2 = keras.layers.Lambda(binary_crossentropy, output_shape=(1,))([input2, out_arm_R,input]) #
loss_out3 = keras.layers.Lambda(binary_crossentropy1, output_shape=(1,))([input3, out_leg_L,out_leg_R,input])
loss_out4 = keras.layers.Lambda(binary_crossentropy1, output_shape=(1,))([input4, out_leg_R,out_leg_L,input])
...
print(f.evaluate(data.x,y, batch_size=1))
x_test=data.x
print(f.predict(x_test, batch_size=1))